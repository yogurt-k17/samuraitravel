...
@SpringBootTest // テスト用のクラス
@AutoConfigureMockMvc // MockMvcを自動生成（テスト用のMvc）
@ActiveProfiles("test") // application-test.propertiesを指定
public class AdminHouseControllerTest {
	...
	@Test
	public void 未ログインの場合は管理者用の民宿一覧ページからログインページにリダイレクトする() throws Exception { // 例外を投げる（エラーで落ちないように）
		mockMvc.perform(get("/admin/houses")) // GETリクエストを模擬的に行う
				.andExpect(status().is3xxRedirection()) // HTTPステータスコード3xx（リダイレクト系）であることを期待する
	...
	public void 一般ユーザーとしてログイン済みの場合は管理者用の民宿一覧ページが表示されずに403エラーが発生する() throws Exception {
		mockMvc.perform(get("/admin/houses"))
				.andExpect(status().isForbidden()); // アクセスはできるが閲覧権限がない
	}
	...
	public void 未ログインの場合は民宿を登録せずにログインページにリダイレクトする() throws Exception {
		...
		String fileName = filePath.getFileName().toString(); // ファイル名をString型で取得
		String fileType = Files.probeContentType(filePath); // 指定したパスのMIMEタイプ(jpeg/imageなど)を取得
		byte[] fileBytes = Files.readAllBytes(filePath); // ファイル内容をバイト配列として返す

		MockMultipartFile imageFile = new MockMultipartFile( // テスト用疑似ファイル
		...
		mockMvc.perform(MockMvcRequestBuilders.multipart("/admin/houses/create").file(imageFile)
				.with(csrf()) // HTTPリクエストにCSRFトークンを含める
				.param("name", "テスト民宿名")
	...
	public void 未ログインの場合は民宿を更新せずにログインページにリダイレクトする() throws Exception {
		...
		assertThat(optionalHouse).isPresent(); // オブジェクトが存在するかチェック
